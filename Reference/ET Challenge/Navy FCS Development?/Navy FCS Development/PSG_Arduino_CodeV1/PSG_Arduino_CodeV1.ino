...
void setup(){
  ...
  pinMode(ActioningTheWeaponPin, OUTPUT);
...
void loop() {   
  ...
  if (Serial.available() >= 0) {       // check to see if a new set of commands is available
    watchdog = 0;
    processingMessage += char(indicator = Serial.read());         // read first byte in buffer
    if(indicator == 'a') {             // check for 'a' (indicates start of message)
      idle = false; 
      idleCounter = 0;
      digitalWrite(USBIndicatorLEDPin, HIGH);   // light up the USB indicator LED
      processingMessage += char(x100byte = Serial.read());         // read the message, byte by byte
      processingMessage += char(x010byte = Serial.read());         //
      processingMessage += char(x001byte = Serial.read());         //
      processingMessage += char(y100byte = Serial.read());         //
      processingMessage += char(y010byte = Serial.read());         //
      processingMessage += char(y001byte = Serial.read());         //
      processingMessage += char(fireByte = Serial.read());         //
      processingMessage += char(fireSelectorByte = Serial.read()); //
      char(fireSelector = int(fireSelectorByte) - 48);  // convert byte to integer
      processingMessage += char(scanningByte = Serial.read());
      serialMessage += ("|Arduino Message = " + processingMessage + "| ");
  ...
  if(idle) {                          // when Arduino is not getting commands from computer...
    Serial.write("Idle #");                // tell the computer that Arduino is here
    ...
    xPosition = processingMessage.substring(1,4).toInt();
    //xPosition = 180 - xPosition; 
    yPosition = processingMessage.substring(4,7).toInt();
    //yPosition = 180 - yPosition;
    serialMessage += ("X Message = " + String(xPosition) + "| " + "Y Message = " + String(yPosition) + "|--#");
...
void assignPins() {
  if(type == "Arduino_bare" || type == "Arduino_Bare") {
    ...
    firingIndicatorLEDPin = 7;              // Arduino pin for firing indicator LED
    ActioningTheWeaponPin = 11;
    USBIndicatorLEDPin = 4;                 // Arduino pin for USB indicator LED
    ...
    electricTriggerPin = 12;                 // Arduino pin for output to trigger MOSFET
    ...
    triggerServoPin = 10;                    // Arduino pin for trigger servo, or output to trigger MOSFET
    electricTriggerPin = 7;                 // Arduino pin for output to trigger MOSFET
    firingIndicatorLEDPin = 10;              // Arduino pin for firing indicator LED
    USBIndicatorLEDPin = 14;                 // Arduino pin for USB indicator LED
    modeIndicatorLEDPin = 13;                // Arduino pin for Mode indicator LED
    reloadSwitchPin = 11;                    // Arduino pin for input from RELOAD switch
    disablePlatePin = 2;                    // Arduino pin for input from disable plate
