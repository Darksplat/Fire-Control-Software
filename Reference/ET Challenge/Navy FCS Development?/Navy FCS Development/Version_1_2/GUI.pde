void initializeGUI() {
  ...
  dl = cp5.addDropdownList("Serial Ports")
    .setPosition(1000, 25)
    .setItemHeight(30)
    .setBarHeight(30)
    .setDefaultValue(999)
...
void updateSerialDropDown(){
 dl.clear();
 dl.addItems(serial);
}

int serialGetValue(){
  int portNumber = (int) Math.floor(dl.getValue());
  if(serial.length > 0){
  return portNumber + 1;
  }
  else return 999;
...
void setDropDownGreen(){
 dl.setColorBackground(color(0,150,0));
 dl.setLabel(serial[serialGetValue() - 1]);
}

void setDropDownRed(){
  dl.setColorBackground(color(150,0,0));
}

void setDropDownDefault(){
  dl.setLabel("Choose Serial Port");
}
