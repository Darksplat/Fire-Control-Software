...
float xAxisServoMaxRotaion = 180.00;//sets the X Axis servo command limits
float yAxisServoMaxRotaion = 180.00;//sets the Y Axis servo command limits
...
int dataFlowTimeWait = 500;//2 seconds
int startTime = 0;//the start time of the timer
int serialTimeout = 0;//point of time where the program will alert that data flow has stoped
boolean arduinoReturnMessage = true;
...
void draw() {
  ...
  if (colorMode) {
  colorDetection();
  sendToArduino(avgX,avgY);
  } else {
    prev.loadPixels();
    motionTracking();
    sendToArduino(motionX,motionY);
  }
}
