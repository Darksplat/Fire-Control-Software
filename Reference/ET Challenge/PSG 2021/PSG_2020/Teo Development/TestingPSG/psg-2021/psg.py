...
class Calibration:
    ...
    def calculate_turret_position(self, target: ScreenCoords) -> (int, int):
        with self.lock:
            ...
            while row < self.NUM_ROWS - 1 and col < self.NUM_COLS - 1:
            ...
            logging.debug(f"Using grid square ({row}, {col}), with corners "\
                ...
                f"ne ({grid['x'][row][col]}, {grid['y'][row][col+1]}) "\
                f"se ({grid['x'][row+1][col]+1}, {grid['y'][row+1][col+1]}) "\
                f"sw ({grid['x'][row+1][col+1]}, {grid['y'][row+1][col]}), "\
    ...
    def run(self):
        while not self.done:
            with self.condition:
                ...
                new_tilt = int((tilt_down_limit - tilt_up_limit)/2)
...
class VideoProcessor(Daemon):
    def __init__(self, controls, calibration, turret, videos, width):
        ...
        if not self.videos:
            ...
            logging.info(f"Found {len(self.videos)} videos to play/loop")
            self.__load_video()
    ...
    def run(self):
        while not self.done:
            ...
            #logging.debug(f"Shape of frame: {frame.shape}")
...
video_processor = VideoProcessor(controls, calibration, controller, videos, video_width)
